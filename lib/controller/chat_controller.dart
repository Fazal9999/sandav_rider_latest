...
import 'package:sandav/data/model/response/message_model.dart';
...
class ChatController extends GetxController implements GetxService {
    ...
    if (response.statusCode == 200) {
      ...
      for (int index = 0;
        ...
        if (_searchConversationModel.conversations[index].receiverType ==
          ...
          _sender = false;
          break;
        } else if (_searchConversationModel.conversations[index].receiverType ==
            UserType.admin.name) {
          _index = index;
          _sender = true;
          break;
    ...
    if (_response != null &&
      ...
      if (offset == 1) {
        ...
        _messageModel.totalSize =
            MessageModel.fromJson(_response.body).totalSize;
        _messageModel.offset = MessageModel.fromJson(_response.body).offset;
        _messageModel.messages
            .addAll(MessageModel.fromJson(_response.body).messages);
      }
    } else {
  ...
  Future<Response> sendMessage(
    ...
    if (notificationBody == null || notificationBody.adminId != null) {
      _response = await chatRepo.sendMessage(
          message, _myImages, 0, UserType.admin, null);
    } else if (notificationBody.restaurantId != null) {
      _response = await chatRepo.sendMessage(message, _myImages,
          notificationBody.restaurantId, UserType.vendor, conversationID);
    } else if (notificationBody.deliverymanId != null) {
      _response = await chatRepo.sendMessage(
          message,
    ...
    if (_response.statusCode == 200) {
      ...
      if (_conversationModel != null &&
          ...
          (_messageModel.conversation.senderType == UserType.admin.name ||
              _messageModel.conversation.receiverType == UserType.admin.name)) {
